...
export default function createInstantSearchManager({
  ...
  let skip = false;
  let stalledSearchTimer: number | undefined = undefined;
  ...
  function search() {
    if (!skip) {
      ...
      const requestPromises = derivedParameters.map(
        ({ indexId, parameters }) => {
          ...
          return new Promise((resolve, reject) => {
            derivedHelper
              .on('result', event => {
                handleSearchSuccess({ indexId })(event);
                resolve();
              })
              .on('error', event => {
  ...
  function handleNewSearch() {
    if (!stalledSearchTimer) {
      // @ts-ignore confsion between node & browser types
